    ...
    <div class="flex justify-center">
      <div class="flex-1">
        <div class="w-auto border rounded">
          ...
          <div class="p-4">
            <div v-if="success.value" class="mx-auto text-center py-6 bg-green-500 rounded border-0 text-white relative">
            ...
            <router-link v-if="success.value" :to="{ name: 'auth.login' }" class="w-full">
              <AppButton primary :text="$t('signin')" class="mt-4 w-full">
                <template #text>
                  {{ $t('signin') }}
                  <ArrowLeftOnRectangleIcon class="ml-2 h-6 w-6"></ArrowLeftOnRectangleIcon>
                </template>
              </AppButton>
            </router-link>

            <div v-if="!success.value">
              <form @submit.prevent="submitResetPassword" class="mx-auto w-full md:w-10/12 md:p-4">
                <input id="username" name="username" type="hidden" readonly disabled>
                <input v-model="resetForm.email" id="email" type="hidden" autocomplete="email" readonly disabled>
                <div class="my-1 w-full py-2 mb-2 sm:flex sm:items-center sm:justify-between">
                  <label for="password" class="w-4/12 dark:text-gray-100" :class="{
                  ...
                  <div class="w-full sm:mt-0 sm:w-8/1">
                    <input v-model="resetForm.password" id="password" type="password" name="password"
                      autocomplete="new-password"
                      class="mt-3 w-full rounded border border-gray-300 bg-white p-2 focus:outline-none2" :class="{
                        'text-red-500 dark:text-red-500 dark:border-red-500':
                          ((validationErrors?.password) || (validationErrors?.password_confirmation)),
                      }" :disabled="processing.value" />
                    <div v-if="validationErrors?.password" class="absolute">
                      <div v-for="message in validationErrors?.password" class="mt-1 text-xs text-red-500">
                        {{ message }}
                      </div>
                    </div>
                  </div>
                ...
                <div class="my-1 w-full py-2 mb-2 sm:flex sm:items-center sm:justify-between">
                  <label for="password_confirmation" class="w-4/12 dark:text-gray-100" :class="{
                    'text-red-500 dark:text-red-500':
                      ((validationErrors?.password) || (validationErrors?.password_confirmation)),
                  }">
                    {{ $t('confirm_password') }}
                  ...
                  <div class="w-full sm:mt-0 sm:w-8/1">
                    ...
                    <div v-if="validationErrors?.password_confirmation" class="absolute">
                      <div v-for="message in validationErrors?.password_confirmation" class="mt-1 text-xs text-red-500">
                        {{ message }}
                      </div>
                    </div>
                  </div>
                ...
                <div class="my-1 w-full py-2 sm:flex sm:items-center sm:justify-end">
                  <div class="mt-3 flex w-full items-center justify-end sm:mt-0 sm:w-8/12">
                    <AppButton :loading="processing.value" :disabled="processing.value || success"
