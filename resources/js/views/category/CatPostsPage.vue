<template>
  <div class="container">
    ...
    <div class="row mb-2">
      <div v-for="post in posts?.data" :key="post.id + '_' + post.title" class="col-md-6">
        <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
          <div class="col-auto d-none d-lg-block d-md-block">
            <img :src="getImageUrl(post)" class="img-fluid" />
          </div>
          <div class="col p-4 d-flex flex-column position-static">
            <strong v-for="category in post.categories" :key="category.name" class="d-inline-block mb-2 text-primary">
              {{ category.name }}
            </strong>
            <h3 class="mb-0">
              {{ post.title }}
            ...
            <router-link :to="{ name: 'public-posts.details', params: { id: post.id } }" class="stretched-link">
              Continue reading
            </router-link>
          </div>
        </div>
