<template>
  <div id="activity_logs" class="bg-white p-3 dark:bg-slate-800 dark:text-gray-200">
    ...
    <AdminBreadcrumbContainer>
      ...
      <AdminBreadcrumbSep />
      <AdminBreadcrumb routeName="activity_log.index" :text="$t('activity_logs')" />
    </AdminBreadcrumbContainer>
          ...
          <div class="w-auto rounded border dark:border-gray-600">
            ...
            <div class="mx-4 mt-4">
              <div class="mt-2 grid grid-cols-1">
                <input v-model="searchTerm" type="text" name="search" id="search"
                  ...
                  placeholder="Search activities..." @input="applyFilters">
                <MagnifyingGlassIcon
            ...
            <div class="m-4 mb-0">
              ...
              <div class="hidden sm:block">
                <nav class="isolate flex divide-x divide-gray-200 dark:divide-gray-500 rounded-lg shadow"
                  aria-label="Tabs">
                  <div @click.prevent="updateFilter('all')"
                    ...
                    :class="activeFilter === 'all' ? 'cursor-default text-gray-900 bg-gray-200 dark:bg-gray-700 dark:text-gray-100' : 'bg-white text-gray-500 hover:bg-gray-50 hover:text-gray-700 cursor-pointer dark:bg-gray-600 dark:text-gray-100 dark:hover:bg-gray-500'">
                    <span>All</span>
                  ...
                  <div @click.prevent="updateFilter('created')"
                    ...
                    :class="activeFilter === 'created' ? 'cursor-default text-gray-900 bg-gray-200 dark:bg-gray-700 dark:text-gray-100' : 'bg-white text-gray-500 hover:bg-gray-50 hover:text-gray-700 cursor-pointer dark:bg-gray-600 dark:text-gray-100 dark:hover:bg-gray-500'">
                    <span>Created</span>
                    <span aria-hidden="true" class="absolute inset-x-0 bottom-0 h-0.5"
                      :class="activeFilter === 'created' ? 'bg-indigo-500 dark:bg-indigo-600' : 'bg-transparent'" />
                  </div>
                  <div @click.prevent="updateFilter('updated')"
                    class="group relative min-w-0 flex-1 overflow-hidden p-2 text-center text-sm font-medium focus:z-10"
                    :aria-current="activeFilter === 'updated' ? 'page' : ''"
                    :class="activeFilter === 'updated' ? 'cursor-default text-gray-900 bg-gray-200 dark:bg-gray-700 dark:text-gray-100' : 'bg-white text-gray-500 hover:bg-gray-50 hover:text-gray-700 cursor-pointer dark:bg-gray-600 dark:text-gray-100 dark:hover:bg-gray-500'">
                    <span>Updated</span>
                    <span aria-hidden="true" class="absolute inset-x-0 bottom-0 h-0.5"
                      :class="activeFilter === 'updated' ? 'bg-indigo-500 dark:bg-indigo-600' : 'bg-transparent'" />
                  </div>
                  <div @click.prevent="updateFilter('login')"
                    class="group relative min-w-0 flex-1 overflow-hidden p-2 text-center text-sm font-medium focus:z-10"
                    :aria-current="activeFilter === 'login' ? 'page' : ''"
                    :class="activeFilter === 'login' ? 'cursor-default text-gray-900 bg-gray-200 dark:bg-gray-700 dark:text-gray-100' : 'bg-white text-gray-500 hover:bg-gray-50 hover:text-gray-700 cursor-pointer dark:bg-gray-600 dark:text-gray-100 dark:hover:bg-gray-500'">
                    <span>Login</span>
                    <span aria-hidden="true" class="absolute inset-x-0 bottom-0 h-0.5"
                      :class="activeFilter === 'login' ? 'bg-indigo-500 dark:bg-indigo-600' : 'bg-transparent'" />
                  </div>
                  <div @click.prevent="updateFilter('logout')"
                    ...
                    :class="activeFilter === 'logout' ? 'cursor-default text-gray-900 bg-gray-200 dark:bg-gray-700 dark:text-gray-100' : 'bg-white text-gray-500 hover:bg-gray-50 hover:text-gray-700 cursor-pointer dark:bg-gray-600 dark:text-gray-100 dark:hover:bg-gray-500'">
                    <span>Logout</span>
                    <span aria-hidden="true" class="absolute inset-x-0 bottom-0 h-0.5"
            ...
            <div v-if="activities?.data?.length > 0" class="text-center">
              <div class="flex items-center justify-between border-t rounded-b border-gray-200 px-4 py-3 sm:px-4">
                <div class="flex flex-1 justify-between md:hidden">
                  <div @click.prevent="changePage(currentPage - 1)" :class="{ disabled: currentPage === 1 }"
                  ...
                  <div
                    ...
                    :class="{ disabled: currentPage === totalPages }" @click.prevent="changePage(currentPage + 1)">
                    {{ $t('next') }}
                  </div>
                </div>
